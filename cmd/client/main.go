...
//	--token        JWT bearer token (env: BOOT_SERVICE_TOKEN)
...
var bmcDeleteCmd = &cobra.Command{
	Use:   "delete [uid...]",
	Short: "Delete one or more BMC resources",
	Long: `Deletes one or more BMC resources.
...
Each UID is processed one by one as an individual, atomic delete operation. If any deletion fails, the command will still attempt to delete the remaining UIDs and report the errors at the end.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, uid := range args {
			if err := c.DeleteBMC(ctx, uid); err != nil {
				errs = append(errs, fmt.Errorf("failed to delete BMC %q: %w", uid, err))
				continue
			}
...
Examples:
  # Create from stdin
  echo '{"hosts": ["["item1","item2"]"], "macs": ["["item1","item2"]"], "nids": [], "groups": ["["item1","item2"]"], "profile": "example-value", "kernel": "example-value", "initrd": "example-value", "params": "example-value", "priority": 42}' | client bootconfiguration create

  # Create with --spec flag
  client bootconfiguration create --spec '{"hosts": ["["item1","item2"]"], "macs": ["["item1","item2"]"], "nids": [], "groups": ["["item1","item2"]"], "profile": "example-value", "kernel": "example-value", "initrd": "example-value", "params": "example-value", "priority": 42}'
...
Examples:
  # Update from stdin
  echo '{"hosts": ["["item1","item2"]"], "macs": ["["item1","item2"]"], "nids": [], "groups": ["["item1","item2"]"], "profile": "example-value", "kernel": "example-value", "initrd": "example-value", "params": "example-value", "priority": 42}' | client bootconfiguration update <uid>

  # Update with --spec flag
  client bootconfiguration update <uid> --spec '{"hosts": ["["item1","item2"]"], "macs": ["["item1","item2"]"], "nids": [], "groups": ["["item1","item2"]"], "profile": "example-value", "kernel": "example-value", "initrd": "example-value", "params": "example-value", "priority": 42}'
...
var bootconfigurationDeleteCmd = &cobra.Command{
	Use:   "delete [uid...]",
	Short: "Delete one or more BootConfiguration resources",
	Long: `Deletes one or more BootConfiguration resources.
...
Each UID is processed one by one as an individual, atomic delete operation. If any deletion fails, the command will still attempt to delete the remaining UIDs and report the errors at the end.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		var errs []error
		for _, uid := range args {
			if err := c.DeleteBootConfiguration(ctx, uid); err != nil {
				errs = append(errs, fmt.Errorf("failed to delete BootConfiguration %q: %w", uid, err))
				continue
			}
