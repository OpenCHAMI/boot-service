...
func registerResourcePrefixes() error {
	...
	resource.RegisterResourcePrefix("BootConfiguration", "bootconfiguration")
...
func RegisterGeneratedRoutes(r chi.Router) {
	...
	// BMC routes
	r.Route("/bmcs", func(resource chi.Router) {
		resource.Get("/", GetBMCs)
		resource.Post("/", CreateBMC)
		resource.Route("/{uid}", func(item chi.Router) {
			item.Get("/", GetBMC)
			item.Put("/", UpdateBMC)
			item.Patch("/", PatchBMC)
			item.Delete("/", DeleteBMC)

			// Status subresource
			item.Route("/status", func(status chi.Router) {
				status.Put("/", UpdateBMCStatus)
				status.Patch("/", PatchBMCStatus)
			})
		})
	})

	// BootConfiguration routes
	r.Route("/bootconfigurations", func(resource chi.Router) {
		resource.Get("/", GetBootConfigurations)
		resource.Post("/", CreateBootConfiguration)
		resource.Route("/{uid}", func(item chi.Router) {
			item.Get("/", GetBootConfiguration)
			item.Put("/", UpdateBootConfiguration)
			item.Patch("/", PatchBootConfiguration)
			item.Delete("/", DeleteBootConfiguration)

			// Status subresource
			item.Route("/status", func(status chi.Router) {
				status.Put("/", UpdateBootConfigurationStatus)
				status.Patch("/", PatchBootConfigurationStatus)
			})
		})
	})

	// Node routes
	r.Route("/nodes", func(resource chi.Router) {
		resource.Get("/", GetNodes)
		resource.Post("/", CreateNode)
		resource.Route("/{uid}", func(item chi.Router) {
			item.Get("/", GetNode)
			item.Put("/", UpdateNode)
			item.Patch("/", PatchNode)
			item.Delete("/", DeleteNode)

			// Status subresource
			item.Route("/status", func(status chi.Router) {
				status.Put("/", UpdateNodeStatus)
				status.Patch("/", PatchNodeStatus)
			})
		})
	})
