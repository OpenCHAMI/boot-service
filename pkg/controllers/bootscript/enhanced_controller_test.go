...
func TestEnhancedController_HSMIntegration(t *testing.T) {
	...
	t.Run("HSM Stats", func(t *testing.T) {
		...
		if !stats["hsm_healthy"].(bool) {
			t.Error("HSM not reported as healthy")
		}
		
		if stats["total_components"].(int) != 3 {
			t.Errorf("Expected 3 total components, got %d", stats["total_components"].(int))
		}
...
func TestEnhancedController_HSMSyncWorker(t *testing.T) {
	...
	if !stats["hsm_healthy"].(bool) {
		t.Error("HSM not healthy after sync worker start")
	}
